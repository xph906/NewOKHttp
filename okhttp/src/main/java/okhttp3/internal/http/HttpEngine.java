 ...
 * <ol>
 * <li>It is created.
 * <li>The HTTP request message is sent with sendRequest(). Once the request is
 * sent it is an error to modify the request headers. After sendRequest() has
 * been called the request body can be written to if it exists.
 * <li>The HTTP response message is read with readResponse(). After the response
 * has been read the response headers and body can be read. All responses have a
...
public final class HttpEngine {
	...
	/**
	 * The time when the request headers were written, or -1 if they haven't
	 * been written yet.
	 */
	...
	/**
	 * The request to send on the network, or null for no network request. This
	 * is derived from the user request, and customized to support OkHttp
	 ...
	 * for this request. Conditional caching means this may be non-null even
	 * when the network request is non-null. Never modified by OkHttp.
	 */
	...
	/**
	 * @param request
	 *            the HTTP request without a body. The body must be written via
	 *            the engine's request body stream.
	 * @param callerWritesRequestBody
	 *            true for the {@code HttpURLConnection}-style interaction model
	 *            where control flow is returned to the calling application to
	 *            write the request body before the response body is readable.
	 */
	public HttpEngine(OkHttpClient client, Request request,
			boolean bufferRequestBody, boolean callerWritesRequestBody,
			boolean forWebSocket, StreamAllocation streamAllocation,
		...
		this.streamAllocation = streamAllocation != null ? streamAllocation
				: new StreamAllocation(client.connectionPool(), createAddress(
						client, request));
	 ...
	 *             if there was a problem with request setup. Unrecoverable.
	 * @throws RouteException
	 *             if the was a problem during connection via a specific route.
	 *             Sometimes recoverable. See {@link #recover}.
	 * @throws IOException
	 *             if there was a problem while making a request. Sometimes
	 *             recoverable. See {@link #recover(IOException)}.
	 */
	public void sendRequest() throws RequestException, RouteException,
		...
		logger.log(Level.INFO,
				String.format("HTTPEngine Search cache: %dms", t2 - now));
		...
		if (networkRequest != null) {
			...
			userRequest.getRequestTimingANP().setConnSetupStartTimeANP(t3);
			httpStream = connect();
			long t4 = System.currentTimeMillis();
			userRequest.getRequestTimingANP().setConnSetupEndTimeANP(t4);
			logger.log(Level.INFO,
					String.format("HTTPEngine connect: %dms", t4 - t3));
			...
			if (callerWritesRequestBody && permitsRequestBody(networkRequest)
				...
				if (bufferRequestBody) {
					...
					httpStream.writeRequestHeaders(networkRequest);
					requestBodyOut = httpStream.createRequestBody(
	...
	private HttpStream connect() throws RouteException, RequestException,
			IOException {
		boolean doExtensiveHealthChecks = !networkRequest.method()
				.equals("GET");
	...
	private static Response stripBody(Response response) {
		return response != null && response.body() != null ? response
				.newBuilder().body(null).build() : response;
	}
	...
	public BufferedSink getBufferedRequestBody() {
		...
		return requestBody != null ? (bufferedRequestBody = Okio
	 ...
	 * server. Returns a new HTTP engine that should be used for the retry if
	 * {@code e} is recoverable, or null if the failure is permanent. Requests
	 * with a body can only be recovered if the body is buffered.
	 */
	public HttpEngine recover(IOException e, Sink requestBodyOut) {
		...
		return new HttpEngine(client, userRequest, bufferRequestBody,
				callerWritesRequestBody, forWebSocket, streamAllocation,
	 ...
	 * either exhausted or closed. If it is unneeded when this is called, it
	...
	/**
	 * Release any resources held by this engine. Returns the stream allocation
	 * held by this engine, which itself must be used or released.
	 */
	public StreamAllocation close() {
		...
		if (userResponse != null) {
			...
			// If this engine never achieved a response body, its stream
			// allocation is dead.
	 ...
	 * if transparent gzip was both offered by OkHttp and used by the origin
	 * server.
	 *
	 * <p>
	 * In addition to decompression, this will also strip the corresponding
	 * headers. We strip the Content-Encoding header to prevent the application
	 * from attempting to double decompress. We strip the Content-Length header
	 * because it is the length of the compressed content, but the application
	 * is only interested in the length of the uncompressed content.
	 *
	 * <p>
	 * This method should only be used for non-empty response bodies. Response
	 * codes like "304 Not Modified" can include "Content-Encoding: gzip"
	 * without a response body and we will crash if we attempt to decompress the
	 * zero-byte source.
	 */
	private Response unzip(final Response response) throws IOException {
		if (!transparentGzip
				|| !"gzip".equalsIgnoreCase(userResponse
		...
		Headers strippedHeaders = response.headers().newBuilder()
				.removeAll("Content-Encoding").removeAll("Content-Length")
				.build();
		return response
				.newBuilder()
				.headers(strippedHeaders)
				.body(new RealResponseBody(strippedHeaders, Okio
						.buffer(responseBody))).build();
	}
	...
	public static boolean hasBody(Response response) {
		...
		if (OkHeaders.contentLength(response) != -1
				|| "chunked".equalsIgnoreCase(response
	...
	private String cookieHeader(List<Cookie> cookies) {
		...
		for (int i = 0, size = cookies.size(); i < size; i++) {
			...
			cookieHeader.append(cookie.name()).append('=')
					.append(cookie.value());
		}
	...
	class NetworkInterceptorChain implements Interceptor.Chain {
		...
		public Response proceed(Request request) throws IOException {
			...
			if (index > 0) {
				Interceptor caller = client.networkInterceptors()
						.get(index - 1);
				Address address = connection().route().address();

				// Confirm that the interceptor uses the connection we've
				// already prepared.
				if (!request.url().host().equals(address.url().host())
					...
					throw new IllegalStateException("network interceptor "
							+ caller + " must retain the same host and port");
				}

				// Confirm that this is the interceptor's first call to
				// chain.proceed().
				if (calls > 1) {
					throw new IllegalStateException("network interceptor "
							+ caller + " must call proceed() exactly once");
				}
			}

			if (index < client.networkInterceptors().size()) {
				// There's another interceptor in the chain. Call that.
				NetworkInterceptorChain chain = new NetworkInterceptorChain(
						index + 1, request);
				Interceptor interceptor = client.networkInterceptors().get(
						index);
				Response interceptedResponse = interceptor.intercept(chain);

				// Confirm that the interceptor made the required call to
				// chain.proceed().
				if (chain.calls != 1) {
					throw new IllegalStateException("network interceptor "
				...
				if (interceptedResponse == null) {
					throw new NullPointerException("network interceptor "
			...
			// Update the networkRequest with the possibly updated interceptor
			// request.
			networkRequest = request;

			if (permitsRequestBody(request) && request.body() != null) {
				Sink requestBodyOut = httpStream.createRequestBody(request,
			...
			logger.log(Level.INFO,String.format("read responss: %dms ", t2-t1) );
			return response;
		}
	}
	...
	private Response readNetworkResponse() throws IOException {
		...
		Response networkResponse = b.request(networkRequest)
				...
				.header(OkHeaders.RECEIVED_MILLIS,
						Long.toString(System.currentTimeMillis())).build();
		
		
		/*Response networkResponse = httpStream
				.readResponseHeaders()
		...
		if (!forWebSocket) {
			networkResponse = networkResponse.newBuilder()
					.body(httpStream.openResponseBody(networkResponse)).build();
		}

		if ("close".equalsIgnoreCase(networkResponse.request().header(
				"Connection"))
		...
		long t3 = System.currentTimeMillis();
		logger.log(Level.INFO,String.format("readNetworkResponse: %dms %dms ", t2-t1, t3-t2) );
	...
	private Response cacheWritingResponse(final CacheRequest cacheRequest,
		...
		Source cacheWritingSource = new Source() {
			...
			public void close() throws IOException {
				if (!cacheRequestClosed
						&& !discard(this,
								HttpStream.DISCARD_STREAM_TIMEOUT_MILLIS,
								MILLISECONDS)) {
					cacheRequestClosed = true;
					cacheRequest.abort();
				}
	...
	private static boolean validate(Response cached, Response network) {
		...
		if (lastModified != null) {
			Date networkLastModified = network.headers().getDate(
					"Last-Modified");
	...
	private static Headers combine(Headers cachedHeaders, Headers networkHeaders)
		...
		for (int i = 0, size = networkHeaders.size(); i < size; i++) {
			...
			if ("Content-Length".equalsIgnoreCase(fieldName)) {
				continue; // Ignore content-length headers of validating
							// responses.
			}
			if (OkHeaders.isEndToEnd(fieldName)) {
	...
	/**
	 * Figures out the HTTP request to make in response to receiving this
	 * engine's response. This will either add authentication headers or follow
	...
	public Request followUpRequest() throws IOException {
		...
		case HTTP_SEE_OTHER:
			...
			boolean sameScheme = url.scheme()
					.equals(userRequest.url().scheme());
	...
	private static Address createAddress(OkHttpClient client, Request request) {
		...
		return new Address(request.url().host(), request.url().port(),
				client.dns(), client.socketFactory(), sslSocketFactory,
